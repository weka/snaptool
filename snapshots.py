...
class SnapSchedule:
    ...
    def next_snap(self, now):
        if self.nextsnap is None or now > self.nextsnap.nextsnap_dt:
...
class BaseSchedule(object):
    def __init__(self, name):
        ...
        self.name = name
        self.next_snaptime(self.nextsnap_dt)  # initialize algorithm
        # log.debug(f"name is {self.name}")

    def __eq__(self, other):
        if other is None:  # not sure how None works here... is this circular?
            return False
        else:
    ...
    def next_snaptime(self, now):
        if now > self.nextsnap_dt:
            ...
            if now <= temp_datetime:
                ...
                self.nextsnap_dt = datetime.datetime(temp_date.year, temp_date.month, temp_date.day, self.hour,
                                                     self.minute)
...
class HourlySchedule(BaseSchedule):
    ...
    def __init__(self, start_day=0, stop_day=4, start_time=900, stop_time=1700, snap_minute=0, retain=10,
                 upload=False):  # snap_minute == mins past the hour to snap
        self.start_day = start_day
        self.stop_day = stop_day
    ...
    def next_snaptime(self, now):
        ...
        if now > self.nextsnap_dt:
            ...
            now_weekday = now.weekday()
            log.debug(
                f"(hourly) now_weekday = {now_weekday}, self.start_day = {self.start_day}, self.stop_day = {self.stop_day}")

            # start_dt = datetime.datetime(now.year, now.month, now.day, self.start_hour, self.snap_minute)
            if self.start_day <= now_weekday <= self.stop_day:
                # we're within the days they want snaps
                start_dt = datetime.datetime(now.year, now.month, now.day, self.start_hour,
                                             self.snap_minute)  # datetime of first snap today
                stop_dt = datetime.datetime(now.year, now.month, now.day, self.stop_hour,
                                            self.snap_minute)  # datetime of last snap today
                ...
                if start_dt < now < stop_dt:
                    ...
                    if now.minute < self.snap_minute:
                        ...
                        temp_date2 = datetime.datetime(temp_date.year, temp_date.month, temp_date.day, temp_date.hour,
                                                       self.snap_minute)
                        ...
                        if temp_date2 <= stop_dt:
                            ...
                            temp_date2 = datetime.datetime(tomorrow.year, tomorrow.month, tomorrow.day, 0,
                                                           0)  # midnight tonight
                ...
                if day_delta > 7:
                    ...
                    day_delta -= 7
                temp_date = now + datetime.timedelta(days=day_delta)
                target_datetime = datetime.datetime(temp_date.year, temp_date.month, temp_date.day, self.start_hour,
                                                    self.snap_minute)
                self.nextsnap_dt = target_datetime
